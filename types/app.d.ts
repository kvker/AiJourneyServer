...
interface PlusWebviewWebviewStyles {
  ...
  background?: string | undefined;
  /* webview的背景是否透明
    布尔类型，true表示透明，false表示不透明，默认值为false。 webview加载html页面时在性能较低的情况下DOM渲染会先显示白色，如果DOM背景色不为白色时可能会出现闪白的现象，此时可以设置webview背景为透明，同时设置background属性为DOM的背景色来避免闪白。 当background属性值为“transparent”时忽略此值。
    注意：HBuilderX2.7.15+版本支持。 
  */
  webviewBGTransparent?: boolean | undefined;
  /**
   * 窗口顶部背景颜色值
   * 窗口内容滚动到顶部下拉时可见。
   *     可取值：
...
interface PlusPayment {
   ...
   * 参考: [http://www.html5plus.org/doc/zh_cn/payment.html](http://www.html5plus.org/doc/zh_cn/payment.html)
   */
  request(channel?: PlusPaymentPaymentChannel, webviewsstatement?: string, successCB?: (result: PlusPaymentPaymentResult) => void, errorCB?: (result: any) => void): void;
}
