...
module.exports = class Safe implements SafeInterface {
  ...
  async checkText(text: string) {
    ...
    let date = Date.now()
    var genSignature = function (secretKey: string, paramsJson: BaseObject) {
      var sorter = function (paramsJson: BaseObject) {
        var sortedJson: BaseObject = {}
        var sortedKeys = Object.keys(paramsJson).sort() as string[]
    ...
    let ret = await urllib.request('http://as.dun.163.com/v5/text/check', {
      ...rule.default_urllib_options,
      headers: {
      ...
      data: {
        ...data,
        signature: genSignature('ec6c4c411444de66eb00896b7d6ab5db', data)
      },
      dataType: 'json',
    })
    ...
    let suggestion = ret.data.result?.antispam.suggestion
    // 可能为空,必须判断
    let result: any
    if (suggestion === 0) {
      result = {
      ...
      let label_map: { [key: string]: string } = {
        '100001': "色情其他",
        '100002': "色情资源",
        '100003': "性器官",
        '100004': "色情聊骚",
      ...
      result = {
        suggestion: 'block',
        ...ret.data.result,
        labels: detail_data.details ? detail_data.details.hitInfos.map((i: any) => i.value) : [label_map[detail_data.secondLabel as string]]
      }
  ...
  async checkImageDouyin(url: string, second: boolean = false) {
    // console.log({url})
