...
const rule = {
    ...
    is_stage,
    hk_address: process.env.HKAddress,
    bj_api_address: process.env.BJAPIAddress,
    headers: {
    ...
    checkContent(text_data) {
        return __awaiter(this, void 0, void 0, function* () {
            let result = yield this.customCheckContent(text_data.text);
    ...
    blockImage(check_result) {
        return '图片内容不当, 请重试..' + check_result.data.reduce((p, c) => p += '.' + c.msg, '');
    },
    replaceWarnString(str) {
    ...
    getWeatherSuggestion(data) {
        ...
        for (let key in key_map) {
            if (data[key])
                str += `${key_map[key]}:${data[key].brief},${data[key].detail};`;
        }
        return str;
    },
    // 处理积分 扣分和加分
    handleIntegration({ params, user, class_name = 'AGIntegrationHistory', integration_key = 'integration' }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                let integration_history_ret = yield integration_history.save();
