...
function chatglmBase({ req, res, user, attraction_id, send_messages, meta, voice_params = {} }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            let completions_data = { completions: (input) => zhipuai.completionsStd(input.messages), input };
            if (meta) {
                completions_data = { completions: (input) => zhipuai.completionsCharacter(input.messages, meta), input };
            }
            ...
            res.send(rule.success(yield chat.save()));
        }
        catch (error) {
...
router.post('/chatglm/std', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        let user = yield AV.User.become(user_session);
        if (!user) {
...
router.post('/chatglm/std-sse', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const params = req.body;
    ...
    try {
        ...
        let prompt_lc = yield new AV.Query('KSNPrompt').equalTo('objectId', prompt_id).include('context_pointer').first({
            useMasterKey: true
        });
        if (!prompt_lc) {
        ...
        if (attraction_id) {
            let attration_lc = yield new AV.Query('KSNAttraction').equalTo('objectId', attraction_id).first({
                useMasterKey: true
            });
            if (!attration_lc) {
        ...
        if (code_data_id) {
            let code_data_lc = yield new AV.Query('KSNCodeData').equalTo('objectId', code_data_id).include(['area', 'prompt_eazy', 'prompt_default', 'prompt_slow']).first({
                useMasterKey: true
            });
            if (!code_data_lc) {
...
function chatglmBaseSse({ req, res, send_messages, lc_object, meta, voice_params }) {
    return __awaiter(this, void 0, void 0, function* () {
...
function handleModelAnswer(model_type = 'yiyan', data) {
    console.log(data);
    let answer;
    switch (model_type) {
        case 'yiyan':
